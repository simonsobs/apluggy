...
async def test_property(data: st.DataObject) -> None:
    ...
    try:
        async with (stacked := stack(iter(ctxs))) as y:
            ...
            for i in range(n_sends):
                ...
                y = await stacked.gen.asend(sent)
...
def _st_stack(n_ctxs: int, gen_enabled: bool) -> st.SearchStrategy[AStack]:
    # `async_stack_gen_ctxs` is the object to be tested.
